...
ALTER TABLE "environment_leases" ADD CONSTRAINT "environment_leases_task_id_tasks_id_fk" FOREIGN KEY ("task_id") REFERENCES "public"."tasks"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "environment_leases_company_task_idx" ON "environment_leases" USING btree ("company_id","task_id");--> statement-breakpoint
