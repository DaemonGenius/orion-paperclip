...
ALTER TABLE "task_comments" ADD COLUMN IF NOT EXISTS "created_by_run_id" uuid;--> statement-breakpoint
DO $$ BEGIN
...
END $$;--> statement-breakpoint
DO $$ BEGIN
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'feedback_exports_task_id_tasks_id_fk') THEN
  ALTER TABLE "feedback_exports" ADD CONSTRAINT "feedback_exports_task_id_tasks_id_fk" FOREIGN KEY ("task_id") REFERENCES "public"."tasks"("id") ON DELETE cascade ON UPDATE no action;
 END IF;
END $$;--> statement-breakpoint
DO $$ BEGIN
...
END $$;--> statement-breakpoint
DO $$ BEGIN
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'feedback_votes_task_id_tasks_id_fk') THEN
  ALTER TABLE "feedback_votes" ADD CONSTRAINT "feedback_votes_task_id_tasks_id_fk" FOREIGN KEY ("task_id") REFERENCES "public"."tasks"("id") ON DELETE no action ON UPDATE no action;
 END IF;
END $$;--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "feedback_exports_company_task_idx" ON "feedback_exports" USING btree ("company_id","task_id","created_at");--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "feedback_votes_company_task_idx" ON "feedback_votes" USING btree ("company_id","task_id");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "feedback_votes_task_target_idx" ON "feedback_votes" USING btree ("task_id","target_type","target_id");--> statement-breakpoint
...
END $$;--> statement-breakpoint
DO $$ BEGIN
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'task_comments_created_by_run_id_heartbeat_runs_id_fk') THEN
  ALTER TABLE "task_comments" ADD CONSTRAINT "task_comments_created_by_run_id_heartbeat_runs_id_fk" FOREIGN KEY ("created_by_run_id") REFERENCES "public"."heartbeat_runs"("id") ON DELETE set null ON UPDATE no action;
 END IF;
END $$;
