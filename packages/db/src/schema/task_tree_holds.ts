...
export const taskTreeHolds = pgTable(
  "task_tree_holds",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull().references(() => companies.id),
    rootTaskId: uuid("root_task_id").notNull().references(() => tasks.id, { onDelete: "cascade" }),
  ...
  (table) => ({
    companyRootStatusIdx: index("task_tree_holds_company_root_status_idx").on(
      table.companyId,
      table.rootTaskId,
      table.status,
    ),
    companyStatusModeIdx: index("task_tree_holds_company_status_mode_idx").on(table.companyId, table.status, table.mode),
  }),
);
