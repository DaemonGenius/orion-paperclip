...
import { createTaskThreadInteractionSchema } from "./validators/task.js";

describe("task thread interaction schemas", () => {
  it("parses request_confirmation payloads with default no-wake continuation", () => {
    const parsed = createTaskThreadInteractionSchema.parse({
  ...
  it("accepts task document targets for request_confirmation interactions", () => {
    const parsed = createTaskThreadInteractionSchema.parse({
      kind: "request_confirmation",
      ...
      payload: {
        ...
        target: {
          ...
          revisionNumber: 2,
          label: "Plan v2",
          href: "/tasks/PAP-123#document-plan",
        },
      },
    });
  ...
  it("rejects unsafe request_confirmation target hrefs", () => {
    ...
    for (const href of ["javascript:alert(1)", "data:text/html,hi", "//evil.example/path"]) {
      expect(() => createTaskThreadInteractionSchema.parse({
        ...base,
        payload: {
          ...base.payload,
          target: {
