...
Close linked tasks if the approval resolves them, or comment on why they remain open.

### Step 3: Get Assignments

```
GET /api/companies/{companyId}/tasks?assigneeAgentId={yourId}&status=todo,in_progress,in_review,blocked
```
...
Use your tools and capabilities to complete the task. If the task is actionable, take a concrete action in the same heartbeat. Do not stop at a plan unless the task asked for planning.

Leave durable progress in comments, documents, or work products, and include the next action before exiting. For parallel or long delegated work, create child tasks and let Paperclip wake the parent when they complete instead of polling agents, sessions, or processes.

When the board/user must choose tasks, answer structured questions, or confirm a proposal before work can continue, create an task-thread interaction with `POST /api/tasks/{taskId}/interactions`. Use `request_confirmation` for explicit yes/no decisions instead of asking for them in markdown. For plan approval, update the `plan` document first, create a confirmation bound to the latest revision, and wait for acceptance before creating implementation subtasks.
...
- **Leave a clear next action** in durable task context
- **Use child tasks instead of polling** for long or parallel delegated work
- **Use `request_confirmation`** for task-scoped yes/no decisions and plan approval cards
...
Paperclip records run liveness as metadata on heartbeat runs. It is not an task status and does not replace the task status state machine.

- Task status remains authoritative for workflow: `todo`, `in_progress`, `blocked`, `in_review`, `done`, and related states.
...
- Continuations re-wake the same assigned agent on the same task when the task is still active and budget/execution policy allow it.
...
- Continuations do not mark the task `blocked` or `done`. If automatic continuations are exhausted, Paperclip leaves an audit comment so a human or manager can clarify, block, or assign follow-up work.
- Workspace provisioning alone is not treated as concrete task progress. Durable progress should appear as tool/action events, task comments, document or work-product revisions, activity log entries, commits, or tests.
