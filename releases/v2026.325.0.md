...
- **Onboarding seeding** — New projects and tasks are seeded with goal context during onboarding for a better first-run experience.
...
- **Agent history via participation** — Agent task history now uses participation records instead of direct assignment lookups.
...
- **Task workspace reuse** — Workspaces are correctly reused after isolation runs.
...
- **Project filter on tasks** — Tasks list can now be filtered by project. ([#552](https://github.com/paperclipai/paperclip/pull/552), @mvanhorn)
...
- **Priority icon removal** — Removed priority icon from task rows for a cleaner list view.
- **Same-page task toasts** — Suppressed redundant toasts when navigating to an task already on screen.
