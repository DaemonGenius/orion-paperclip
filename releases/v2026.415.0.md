...
- **Faster tasks page first paint** — The tasks list now renders the initial view significantly faster by deferring non-critical work and optimizing workspace lookups.
- **Task detail performance** — Reduced unnecessary rerenders on the task detail page and kept queued task chat mounted to avoid layout thrash during agent runs.
- **Inbox search expansion** — Added an "Other results" section to inbox search, surfacing matches beyond the current filter scope.

## Improvements

- **Properties pane polish** — Workspace link, copy-on-click for identifiers, and an inline parent navigation arrow in the task properties sidebar.
- **Routine UX** — Routine name now appears above the "Run routine" title in the run dialog; routine execution tasks are shown in task lists by default.
...
- **Stranded task diagnostics** — Stranded task comments now include the latest run failure message for faster triage.
- **Tasks search responsiveness** — Debounce and rendering improvements make the tasks page search feel snappier.
- **Live run refresh** — Visible task runs now refresh automatically on status updates without a manual reload.
