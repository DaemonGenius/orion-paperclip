...
- **Execution policies** — Tasks can now carry a review/approval execution policy with multi-stage signoff workflows. Reviewers and approvers are selected per-stage, and Paperclip routes the task through each stage automatically. ([#3222](https://github.com/paperclipai/paperclip/pull/3222))
...
- **Task-to-task navigation** — Faster navigation between tasks with scroll reset, prefetch, and detail-view optimizations. ([#3542](https://github.com/cryppadotta/paperclip/pull/3542))
- **Auto-checkout for scoped wakes** — The agent harness now automatically checks out the scoped task on comment-driven wakes, reducing latency for agent heartbeats. ([#3538](https://github.com/cryppadotta/paperclip/pull/3538))
...
- **Workspace runtime controls** — Start/stop controls, runtime state reconciliation, runtime service JSON textarea improvements, and workspace branch/folder display in the task properties sidebar. ([#3354](https://github.com/paperclipai/paperclip/pull/3354))
...
- **Task detail stability** — Fixed visible refreshes during agent updates, comment post resets, ref update loops, split regressions, main-pane focus on navigation, and other detail-view rerender tasks. ([#3355](https://github.com/paperclipai/paperclip/pull/3355))
...
- **Comment editor sync** — Hardened task comment editor synchronization.
- **Document revisions freshness** — The latest task document revision now stays current in the UI. ([#3342](https://github.com/cryppadotta/paperclip/pull/3342))
