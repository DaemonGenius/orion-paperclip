...
- **Task documents and attachments** — Tasks now support inline document editing, file staging before creation, deep-linked documents, copy and download actions, and live-event refresh. ([#899](https://github.com/paperclipai/paperclip/pull/899))
...
- **Copy-to-clipboard on tasks** — Task detail headers now include a copy button; HTML entities no longer leak into copied text. ([#990](https://github.com/paperclipai/paperclip/pull/990))
- **Me and Unassigned assignee options** — Quick-filter assignee options for the current user and unassigned tasks. ([#990](https://github.com/paperclipai/paperclip/pull/990))
- **Skip pre-filled fields in new task dialog** — Tab order now skips assignee and project fields when they are already populated. ([#990](https://github.com/paperclipai/paperclip/pull/990))
...
- **Dev runner syntax** — Fixed syntax task in plugin dev runner. ([#914](https://github.com/paperclipai/paperclip/pull/914), @gsxdsm)
...
- **Migrations 0030–0037** extend the schema for task documents, execution workspaces, company logos, cost tracking, and plugin enhancements.
