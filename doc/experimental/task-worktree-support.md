# Task worktree support

Status: experimental, runtime-only, not shipping as a user-facing feature yet.

This branch contains the runtime and seeding work needed for task-scoped worktrees:

- project execution workspace policy support
- task-level execution workspace settings
- git worktree realization for isolated task execution
...
- tasks can carry execution workspace settings in the backend
...
- task creation:
  ...
  - isolated task checkout toggle
  - defaulting task execution workspace settings from project policy

- task editing:
  - `ui/src/components/TaskProperties.tsx`
  - task-level workspace mode toggle
  - defaulting task execution workspace settings when project changes
...
- we do not want to expose a partially-baked user-facing feature in tasks, projects, or settings
...
- review wording and defaults for project and task controls
