...
The repo already has `onboard`, `doctor`, `run`, deployment modes, and even agent-oriented onboarding text/skills endpoints, but there are also current onboarding/auth validation tasks and an open “onboard failed” report. That means this is not just polish; it is product-critical. ([GitHub][1])
...
Build a **Command Composer** backed by tasks/comments/approvals, not a separate chat subsystem.

### What we want

- “Talk to the CEO” feeling for the user.
- Every conversation ends up attached to a real company object.
- Strategy discussion can produce tasks, artifacts, and approvals.
...
type ThreadScope = "company" | "project" | "task" | "agent";
```

Examples:

- On dashboard: “Ask the CEO for a hiring plan” → creates a `strategy` task/thread scoped to the company.
- On agent page: “Tell the designer to make this cleaner” → appends an instruction comment to an task or spawns a new delegated task.
- On approval page: “Why are you asking to hire?” → appends a board comment to the approval context.

Add task kinds:

```ts
type TaskKind = "task" | "strategy" | "question" | "decision";
```

### Backend / data model

Prefer extending existing `tasks` rather than creating `chats`:

- `tasks.kind`
- `tasks.scope`
- optional `tasks.target_agent_id`
...
type RunSummary = {
  ...
  delegatedTo: { agentId: string; taskId?: string }[];
  artifactIds: string[];
  warnings: string[];
};
```
...
This gap is already showing up in the repo. Storage is present, attachment endpoints exist, but current tasks show that attachments are still effectively image-centric and comment attachment rendering is incomplete. At the same time, your transcript wants plans, docs, files, and generated web pages surfaced cleanly. ([GitHub][4])
...
- easy discoverability from the task/run/company pages
...
- Accept non-image MIME types for task attachments
- Attach files to comments correctly
- Show file metadata + download/open on task page
...
Task page gets a **Deliverables** section:
...
- preview URL is registered back onto the task as an artifact

### Acceptance criteria

- Agents can attach `.md`, `.txt`, `.json`, `.csv`, `.pdf`, and `.html`.
- Users can open/download them from the task page.
- A generated static site can be opened from an task without hunting through the filesystem.
...
- **operator**: create/update tasks, interact with agents, view artifacts
- **viewer**: read-only
...
This is a product behavior task, not a UI nicety. If agents cannot keep working or accept course correction without restarting, the autonomy model feels fake.
...
This is probably the most important immediate workstream. The transcript says token burn is the highest pain, and the repo currently has active tasks around budget enforcement evidence, onboarding/auth validation, and circuit-breaker style waste prevention. Public docs already promise hard budgets, and the task tracker is pointing at the missing operational protections. ([GitHub][6])
...
- no task/status/comment progress for N runs
- N failures in a row
...
Paperclip should manage the **task → workspace → preview/PR → review handoff** lifecycle, but leave diffs/review/merge to external tools.
...
- For git-backed projects with `prPerTask=true`, one task maps to one isolated branch/worktree
...
- When a project is in PR mode, the task contains branch/worktree/preview/PR metadata.
...
The roadmap already includes plugins, GitHub discussions are active around it, and there is an open task proposing an SSE bridge specifically to enable streaming plugin UIs such as chat, logs, and monitors. This is exactly the right place for optional surfaces. ([GitHub][1])
...
interface PluginManifest {
  ...
  surfaces: ("company_home" | "task_panel" | "agent_panel" | "sidebar")[];
  workerEntry: string;
  uiEntry: string;
}
```
...
- Current task pressure: onboarding failure, auth/onboarding validation, budget enforcement, circuit breaker, attachment gaps, plugin chat. ([GitHub][3])
...
[3]: https://github.com/paperclipai/paperclip/tasks/704 "https://github.com/paperclipai/paperclip/tasks/704"
...
[6]: https://github.com/paperclipai/paperclip/tasks/692 "https://github.com/paperclipai/paperclip/tasks/692"
