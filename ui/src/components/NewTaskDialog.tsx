...
const DRAFT_KEY = "paperclip:task-draft";
...
const TASK_OVERRIDE_ADAPTER_TYPES = new Set(["claude_local", "codex_local", "opencode_local"]);
const STAGED_FILE_ACCEPT = "image/*,application/pdf,text/plain,text/markdown,application/json,text/csv,text/html,.md,.markdown";

const TASK_THINKING_EFFORT_OPTIONS = {
  claude_local: [
    { value: "", label: "Default" },
    { value: "low", label: "Low" },
...
function buildAssigneeAdapterOverrides(input: {
  ...
  if (!adapterType || !TASK_OVERRIDE_ADAPTER_TYPES.has(adapterType)) {
    return null;
  }
...
const statuses = [
  { value: "backlog", label: "Backlog", color: taskStatusText.backlog ?? taskStatusTextDefault },
  { value: "todo", label: "Todo", color: taskStatusText.todo ?? taskStatusTextDefault },
  { value: "in_progress", label: "In Progress", color: taskStatusText.in_progress ?? taskStatusTextDefault },
  { value: "in_review", label: "In Review", color: taskStatusText.in_review ?? taskStatusTextDefault },
  { value: "done", label: "Done", color: taskStatusText.done ?? taskStatusTextDefault },
];
...
function taskExecutionWorkspaceModeForExistingWorkspace(mode: string | null | undefined) {
...
export function NewTaskDialog() {
  ...
  const parentExecutionWorkspaceId = newTaskDefaults.executionWorkspaceId ?? "";
  const parentExecutionWorkspaceLabel = newTaskDefaults.parentExecutionWorkspaceLabel ?? parentExecutionWorkspaceId;
  ...
  const { data: reusableExecutionWorkspaces } = useQuery({
    ...
    enabled: Boolean(effectiveCompanyId) && newTaskOpen && Boolean(projectId),
  });
  ...
  const supportsAssigneeOverrides = Boolean(
    assigneeAdapterType && TASK_OVERRIDE_ADAPTER_TYPES.has(assigneeAdapterType),
  );
  ...
  const { data: assigneeAdapterModels } = useQuery({
    ...
    enabled: Boolean(effectiveCompanyId) && newTaskOpen && supportsAssigneeOverrides,
  });
  ...
  const createTask = useMutation({
    mutationFn: async ({
      ...
      ...data
    }: { companyId: string; stagedFiles: StagedTaskFile[] } & Record<string, unknown>) => {
  ...
  useEffect(() => {
    ...
    if (newTaskDefaults.parentId) {
      ...
      const defaultProjectWorkspaceId = newTaskDefaults.projectWorkspaceId
        ?? defaultProjectWorkspaceIdForProject(defaultProject);
      const defaultExecutionWorkspaceMode = newTaskDefaults.executionWorkspaceId
        ? "reuse_existing"
        : (newTaskDefaults.executionWorkspaceMode ?? defaultExecutionWorkspaceModeForProject(defaultProject));
      setTitle(newTaskDefaults.title ?? "");
      setDescription(newTaskDefaults.description ?? "");
      ...
      setAssigneeValue(assigneeValueFromSelection(newTaskDefaults));
      setAssigneeModelOverride("");
      setAssigneeThinkingEffort("");
      setAssigneeChrome(false);
      setExecutionWorkspaceMode(defaultExecutionWorkspaceMode);
      setSelectedExecutionWorkspaceId(newTaskDefaults.executionWorkspaceId ?? "");
      executionWorkspaceDefaultProjectId.current = defaultProjectId || null;
    } else if (newTaskDefaults.title) {
      setTitle(newTaskDefaults.title);
      setDescription(newTaskDefaults.description ?? "");
      setStatus(newTaskDefaults.status ?? "todo");
      setPriority(newTaskDefaults.priority ?? "");
      ...
      setAssigneeValue(
        newTaskDefaults.assigneeAgentId || newTaskDefaults.assigneeUserId
          ? assigneeValueFromSelection(newTaskDefaults)
  ...
  }, [newTaskOpen, newTaskDefaults, orderedProjects]);
  ...
  useEffect(() => {
    ...
    const validThinkingValues =
      assigneeAdapterType === "codex_local"
        ? TASK_THINKING_EFFORT_OPTIONS.codex_local
  ...
  function handleSubmit() {
    if (!effectiveCompanyId || !title.trim() || createTask.isPending) return;
    ...
    const requestedExecutionWorkspaceMode =
      executionWorkspaceMode === "reuse_existing"
        ? taskExecutionWorkspaceModeForExistingWorkspace(selectedReusableExecutionWorkspace?.mode)
        : executionWorkspaceMode;
  ...
  const isUsingParentExecutionWorkspace = isSubTaskMode && parentExecutionWorkspaceId
    ? executionWorkspaceMode === "reuse_existing" && selectedExecutionWorkspaceId === parentExecutionWorkspaceId
    : false;
  const showParentWorkspaceWarning = isSubTaskMode
  ...
  useEffect(() => {
    if (!newTaskOpen || !projectId || executionWorkspaceDefaultProjectId.current === projectId) {
      return;
    }
    ...
    setSelectedExecutionWorkspaceId("");
  }, [newTaskOpen, orderedProjects, projectId]);
