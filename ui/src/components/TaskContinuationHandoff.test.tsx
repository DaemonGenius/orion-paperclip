...
import { TaskContinuationHandoff } from "./TaskContinuationHandoff";
...
function createHandoffDocument(): TaskDocument {
  return {
    id: "document-handoff",
    companyId: "company-1",
    taskId: "task-1",
...
describe("TaskContinuationHandoff", () => {
  ...
  it("renders compact metadata by default with copy access", async () => {
    ...
    await act(async () => {
      root.render(<TaskContinuationHandoff document={handoff} />);
    });
  ...
  it("expands and anchors the handoff body when focused from a document deep link", async () => {
    ...
    await act(async () => {
      root.render(<TaskContinuationHandoff document={createHandoffDocument()} focusSignal={1} />);
    });

    expect(container.querySelector(`#document-${TASK_CONTINUATION_SUMMARY_DOCUMENT_KEY}`)).toBeTruthy();
