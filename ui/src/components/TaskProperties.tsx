...
import { resolveTaskFilterWorkspaceId } from "../lib/task-filters";
...
function isMainTaskWorkspace(input: {
  task: Pick<Task, "projectWorkspaceId" | "currentExecutionWorkspace">;
  project: Pick<Project, "primaryWorkspace" | "workspaces"> | null | undefined;
}) {
  const workspace = input.task.currentExecutionWorkspace ?? null;
  const primaryWorkspaceId = primaryWorkspaceIdForProject(input.project);
  const linkedProjectWorkspaceId = workspace?.projectWorkspaceId ?? input.task.projectWorkspaceId ?? null;
  if (workspace) {
...
function tasksWorkspaceFilterHref(workspaceId: string) {
  ...
  return Object.entries(relations)
    .map(([label, refs]) => ({
      ...
      refs: Array.isArray(refs)
        ? refs.filter((ref): ref is { pageId: string; title?: string | null; url?: string | null } => (
          Boolean(ref) && typeof ref === "object" && typeof (ref as { pageId?: unknown }).pageId === "string"
        ))
        : [],
    }))
...
function ReadOnlyTextValue({ value, mono }: { value: string; mono?: boolean }) {
  return (
    <span className={cn("text-sm min-w-0 break-words", mono && "font-mono text-xs")}>{value}</span>
  );
}

function ReadOnlyLinkValue({ value }: { value: string }) {
  return (
    <a
      href={value}
      target="_blank"
      rel="noreferrer"
      className="inline-flex min-w-0 items-start gap-1 text-sm text-primary hover:underline"
    >
...
export function TaskProperties({
  ...
  const currentProject = task.projectId
    ? orderedProjects.find((project) => project.id === task.projectId) ?? null
    : null;
  const taskProject = task.project ?? currentProject;
  const isolatedWorkspacesEnabled = experimentalSettings?.enableIsolatedWorkspaces === true;
  const taskUsesMainWorkspace = useMemo(
    () => isMainTaskWorkspace({ task, project: taskProject }),
    [task, taskProject],
  );
  const workspaceFilterId = useMemo(() => {
    if (!isolatedWorkspacesEnabled) return null;
    if (taskUsesMainWorkspace) return null;
    return resolveTaskFilterWorkspaceId(task);
  }, [isolatedWorkspacesEnabled, task, taskUsesMainWorkspace]);
  const showWorkspaceDetailLink = Boolean(task.executionWorkspaceId) && !taskUsesMainWorkspace;
  const liveWorkspaceService = useMemo(() => {
    if (taskUsesMainWorkspace) return null;
    return runningRuntimeServiceWithUrl(task.currentExecutionWorkspace?.runtimeServices);
  }, [task.currentExecutionWorkspace?.runtimeServices, taskUsesMainWorkspace]);
  ...
  const otherUserOptions = useMemo(
    () => buildCompanyUserInlineOptions(companyMembers?.users, { excludeUserIds: [currentUserId, task.createdByUserId] }),
    [companyMembers?.users, currentUserId, task.createdByUserId],
  );
  ...
  const assigneeUserLabel = userLabel(task.assigneeUserId);
  const creatorUserLabel = userLabel(task.createdByUserId);
  const selectedAssigneeValue = task.assigneeAgentId
  ...
  const nextRunnableExecutionStage = (() => {
    if (task.executionState?.status === "changes_requested" && task.executionState.currentStageType) {
      return task.executionState.currentStageType;
    }
    if (task.executionState) return null;
  ...
  const labelsTrigger = selectedTaskLabels.length > 0 ? (
    <div className="flex items-center gap-1 flex-wrap">
      ...
      {selectedTaskLabels.length > 3 && (
        <span className="text-xs text-muted-foreground">+{selectedTaskLabels.length - 3}</span>
      )}
    </div>
  ) : (
    <>
  ...
  const assigneePickerOptions = orderItemsBySelectedAndRecent(
    [
      ...
      ...(currentUserId
        ...
        : []),
      ...(task.createdByUserId && task.createdByUserId !== currentUserId
        ? [{
            id: `user:${task.createdByUserId}`,
            kind: "user" as const,
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        {task.createdByUserId && task.createdByUserId !== currentUserId && (
          <button
            className={cn(
              "flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50",
              values.includes(`user:${task.createdByUserId}`) && "bg-accent",
            )}
            onClick={() => toggleExecutionParticipant(stageType, `user:${task.createdByUserId}`)}
          >
            <User className="h-3 w-3 shrink-0 text-muted-foreground" />
  ...
  const projectTrigger = task.projectId ? (
    <>
      <span
        ...
        style={{ backgroundColor: orderedProjects.find((p) => p.id === task.projectId)?.color ?? "#6366f1" }}
      />
      <span className="text-sm break-words min-w-0">{projectName(task.projectId)}</span>
    </>
  ) : (
    <>
  ...
  const parentTitle = task.ancestors?.[0]?.title ?? currentParentTask?.title ?? task.parentId?.slice(0, 8);
    ...
    <>
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        <button
          ...
          onClick={() => onUpdate({ blockedByTaskIds: [] })}
        >
          No blockers
        </button>
  ...
  return (
    <div className="space-y-4">
        ...
        {showImportedNotionProperties ? (
          <>
            ...
            <div className="space-y-1">
              ...
              {displayOptional(task.blockedByText) ? (
                <PropertyRow label="Blocked By"><ReadOnlyTextValue value={displayOptional(task.blockedByText)!} /></PropertyRow>
              ) : null}
              {displayOptional(task.dueDate) ? (
                <PropertyRow label="Due Date"><ReadOnlyTextValue value={displayOptional(task.dueDate)!} /></PropertyRow>
              ) : null}
              {displayOptional(task.layer) ? (
                <PropertyRow label="Layer"><ReadOnlyTextValue value={displayOptional(task.layer)!} /></PropertyRow>
              ) : null}
              {displayOptional(task.module) ? (
                <PropertyRow label="Module"><ReadOnlyTextValue value={displayOptional(task.module)!} /></PropertyRow>
              ) : null}
              {displayOptional(task.repoPath) ? (
                <PropertyRow label="Repo Path"><ReadOnlyTextValue value={displayOptional(task.repoPath)!} mono /></PropertyRow>
              ) : null}
              {displayOptional(task.riskLevel) ? (
                <PropertyRow label="Risk"><ReadOnlyTextValue value={displayOptional(task.riskLevel)!} /></PropertyRow>
              ) : null}
              {displayOptional(task.sprintPhase) ? (
                <PropertyRow label="Sprint"><ReadOnlyTextValue value={displayOptional(task.sprintPhase)!} /></PropertyRow>
              ) : null}
              {displayOptional(task.taskType) ? (
                <PropertyRow label="Type"><ReadOnlyTextValue value={displayOptional(task.taskType)!} /></PropertyRow>
              ) : null}
              {displayOptional(task.routeMode) ? (
                <PropertyRow label="Route"><ReadOnlyTextValue value={displayOptional(task.routeMode)!} /></PropertyRow>
              ) : null}
              {displayOptional(task.reqId) ? (
                <PropertyRow label="REQ ID"><ReadOnlyTextValue value={displayOptional(task.reqId)!} mono /></PropertyRow>
              ) : null}
              {displayOptional(task.prState) ? (
                <PropertyRow label="PR State"><ReadOnlyTextValue value={displayOptional(task.prState)!} /></PropertyRow>
              ) : null}
              {displayOptional(task.prUrl) ? (
                <PropertyRow label="PR URL"><ReadOnlyLinkValue value={displayOptional(task.prUrl)!} /></PropertyRow>
              ) : null}
              {displayOptional(task.agentConfidenceLevel) ? (
                <PropertyRow label="Confidence"><ReadOnlyTextValue value={displayOptional(task.agentConfidenceLevel)!} /></PropertyRow>
              ) : null}
              {importedNotionRelationEntries.map((entry) => (
                    ...
                    {entry.refs.map((ref) => (
                      ref.url ? (
                        <a
                          ...
                          className="inline-flex min-w-0 items-start gap-1 text-sm text-primary hover:underline"
                        >
                          <span className="min-w-0 break-words">{ref.title ?? ref.pageId}</span>
                          <ExternalLink className="mt-1 h-3 w-3 shrink-0" />
                        </a>
                      ) : (
                        <ReadOnlyTextValue key={ref.pageId} value={ref.title ?? ref.pageId} />
                      )
                    ))}
                  </div>
                </PropertyRow>
      ...
      {liveWorkspaceService || task.currentExecutionWorkspace?.branchName || task.currentExecutionWorkspace?.cwd || task.executionWorkspaceId ? (
        <>
          ...
          <div className="space-y-1">
            ...
            {showWorkspaceDetailLink && task.executionWorkspaceId && (
              <PropertyRow label="Workspace">
                <Link
                  to={`/execution-workspaces/${task.executionWorkspaceId}`}
            ...
            {workspaceFilterId && (
              <PropertyRow label="Tasks">
                <Link
                  to={tasksWorkspaceFilterHref(workspaceFilterId)}
                  className="text-sm text-primary hover:underline inline-flex items-center gap-1"
                >
            ...
            {task.currentExecutionWorkspace?.branchName && (
              <PropertyRow label="Branch">
                <TruncatedCopyable
                  value={task.currentExecutionWorkspace.branchName}
                  icon={GitBranch}
                />
              </PropertyRow>
            )}
            {task.currentExecutionWorkspace?.cwd && (
              <PropertyRow label="Folder">
                <TruncatedCopyable
                  value={task.currentExecutionWorkspace.cwd}
                  icon={FolderOpen}
                />
              </PropertyRow>
            )}
      ...
      <div className="space-y-1">
        {(task.createdByAgentId || task.createdByUserId) && (
          <PropertyRow label="Created by">
            {task.createdByAgentId ? (
              <Link
                to={`/agents/${task.createdByAgentId}`}
                className="hover:underline"
              >
                <Identity name={agentName(task.createdByAgentId) ?? task.createdByAgentId.slice(0, 8)} size="sm" />
              </Link>
            ) : (
              <>
                <User className="h-3.5 w-3.5 text-muted-foreground" />
