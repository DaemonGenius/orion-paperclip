...
import type { Task, RunLivenessState } from "@paperclipai/shared";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import type { RunForTask } from "../api/activity";
import type { ActiveRunForTask } from "../api/heartbeats";
import { TaskRunLedgerContent } from "./TaskRunLedger";
...
function createRun(overrides: Partial<RunForTask> = {}): RunForTask {
  return {
    runId: "run-00000000",
    status: "succeeded",
    agentId: "agent-1",
...
function createActiveRun(overrides: Partial<ActiveRunForTask> = {}): ActiveRunForTask {
  return {
    id: "run-live-1",
    status: "running",
    invocationSource: "assignment",
    ...
    outputSilence: {
      ...
      snoozedUntil: null,
      evaluationTaskId: "task-eval-1",
      evaluationTaskIdentifier: "PAP-404",
      evaluationTaskAssigneeAgentId: "agent-owner",
    },
    ...overrides,
  };
}

function renderLedger(props: Partial<ComponentProps<typeof TaskRunLedgerContent>> = {}) {
  render(
    <TaskRunLedgerContent
      runs={props.runs ?? []}
      liveRuns={props.liveRuns}
      activeRun={props.activeRun}
      taskStatus={props.taskStatus ?? "in_progress"}
...
describe("TaskRunLedger", () => {
  ...
  it("surfaces active and completed child task summaries", () => {
    renderLedger({
      childTasks: [
        createTask({ id: "child-1", identifier: "PAP-2", title: "Implement worker handoff", status: "in_progress" }),
        createTask({ id: "child-2", identifier: "PAP-3", title: "Verify final report", status: "done" }),
        createTask({ id: "child-3", identifier: "PAP-4", title: "Cancelled experiment", status: "cancelled" }),
      ],
    });
