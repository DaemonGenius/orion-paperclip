...
vi.mock("../lib/task-chat-scroll", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../lib/task-chat-scroll")>();
  return {
    ...actual,
...
vi.mock("./TaskLinkQuicklook", () => ({
  TaskLinkQuicklook: ({
    ...
    <a href={to} data-task-path-id={taskPathId} className={className}>
      {children}
    </a>
  ),
}));
...
function createSuggestedTasksInteraction(
  ...
  return {
    ...
    summary: "Preview the next task tree before accepting it.",
    status: "pending",
...
describe("TaskChatThread", () => {
  ...
  it("shows unresolved blocker context above the composer", () => {
    ...
    expect(container.querySelector('[data-task-path-id="PAP-1723"]')).not.toBeNull();
  ...
  it("shows terminal blocker context when an immediate blocker is transitively blocked", () => {
    ...
    expect(container.querySelector('[data-task-path-id="PAP-2201"]')).not.toBeNull();
  ...
  it("stores and restores the composer draft per task key", () => {
    vi.useFakeTimers();
    const root = createRoot(container);
      ...
      root.render(
        <MemoryRouter>
          <TaskChatThread
            ...
            liveRuns={[]}
            onAdd={async () => {}}
            draftKey="task-chat-draft:test-1"
            enableLiveTranscriptPolling={false}
          />
        </MemoryRouter>,
      );
    ...
    expect(localStorage.getItem("task-chat-draft:test-1")).toBe("Draft survives refresh");
  ...
  it("keeps the composer floating with a capped editor height", () => {
    ...
    const dock = container.querySelector('[data-testid="task-chat-composer-dock"]') as HTMLDivElement | null;
    ...
    const composer = container.querySelector('[data-testid="task-chat-composer"]') as HTMLDivElement | null;
  ...
  it("shows full-composer drop instructions while dragging files over the task composer", () => {
    const root = createRoot(container);
    ...
    expect(container.querySelector('[data-testid="task-chat-composer-drop-overlay"]')).not.toBeNull();
  ...
  it("shows non-image attachment upload state in the composer after a drop", async () => {
    ...
    const onAttachImage = vi.fn(async (file: File) => ({
      ...
      assetId: "asset-1",
      provider: "local_disk",
      objectKey: "tasks/task-1/report.pdf",
    ...
    const attachmentList = container.querySelector('[data-testid="task-chat-composer-attachments"]');
  ...
  it("renders the bottom spacer with zero height until the user has submitted", () => {
    ...
    const spacer = container.querySelector('[data-testid="task-chat-bottom-spacer"]') as HTMLDivElement | null;
  ...
  it("omits the bottom spacer when the composer is hidden", () => {
    ...
    const spacer = container.querySelector('[data-testid="task-chat-bottom-spacer"]');
  ...
  it("hides the reopen control and infers reopen for closed agent-assigned task replies", async () => {
    const root = createRoot(container);
  ...
  it("uses company profile data to distinguish the current user from other humans", () => {
    ...
    expect(resolveTaskChatHumanAuthor({
      authorName: "You",
      authorUserId: "user-1",
      currentUserId: "user-1",
      userProfileMap,
