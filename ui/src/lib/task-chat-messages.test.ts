...
import type { SuggestTasksInteraction } from "./task-thread-interactions";
...
describe("buildAssistantPartsFromTranscript", () => {
  it("maps assistant text, reasoning, and tool activity while omitting noisy stderr", () => {
    const result = buildAssistantPartsFromTranscript([
      ...
      { kind: "thinking", ts: "2026-04-06T12:00:02.000Z", text: "Need to inspect files." },
      {
        ...
        name: "read_file",
        toolUseId: "tool-1",
        input: { path: "ui/src/pages/TaskDetail.tsx" },
      },
      {
        kind: "tool_result",
  ...
  it("preserves transcript ordering when text and tool activity are interleaved", () => {
    const result = buildAssistantPartsFromTranscript([
      { kind: "assistant", ts: "2026-04-06T12:00:00.000Z", text: "First." },
      {
        ...
        name: "read_file",
        toolUseId: "tool-1",
        input: { path: "ui/src/components/TaskChatThread.tsx" },
      },
      ...
      { kind: "thinking", ts: "2026-04-06T12:00:04.000Z", text: "Need one more check." },
      {
        ...
        name: "write_file",
        toolUseId: "tool-2",
        input: { path: "ui/src/lib/task-chat-messages.ts" },
      },
      {
        kind: "tool_result",
  ...
  it("preserves diff transcript output as a fenced diff block", () => {
    const result = buildAssistantPartsFromTranscript([
      ...
      { kind: "diff", ts: "2026-04-06T12:00:01.000Z", changeType: "file_header", text: "ui/src/lib/task-chat-messages.ts" },
    ...
    expect(result.parts).toMatchObject([
      { type: "text", text: "Applied the patch." },
      {
        ...
        text: [
          "```diff",
          "ui/src/lib/task-chat-messages.ts",
...
describe("buildTaskChatMessages", () => {
  ...
  it("orders events before comments and appends active live runs as running assistant messages", () => {
    ...
    const timelineEvents: TaskTimelineEvent[] = [
      {
        id: "event-1",
    ...
    const linkedRuns: TaskChatLinkedRun[] = [
      {
        runId: "run-history-1",
        status: "succeeded",
        agentId: "agent-1",
  ...
  it("keeps succeeded runs as assistant messages when transcript output exists", () => {
    ...
    const messages = buildTaskChatMessages({
      ...
      transcriptsByRunId: new Map([
        [
          "run-history-1",
          [
            { kind: "thinking", ts: "2026-04-06T12:01:10.000Z", text: "Checking the current task thread." },
    ...
    expect(messages[0]?.content).toMatchObject([
      { type: "reasoning", text: "Checking the current task thread." },
      { type: "text", text: "Updated the thread renderer." },
    ]);
  });

  it("compacts long run transcripts in task chat while preserving matching tool context", () => {
    ...
    const transcript = [
      ...
      })),
      {
        ...
        name: "search",
        toolUseId: "tool-keep",
        input: { query: "task chat virtualization" },
      },
  ...
  it("keeps the same assistant message id when a live run becomes a cancelled historical run", () => {
    const liveMessages = buildTaskChatMessages({
      comments: [],
      timelineEvents: [],
      linkedRuns: [],
      liveRuns: [
    ...
    const cancelledMessages = buildTaskChatMessages({
      comments: [],
      timelineEvents: [],
      linkedRuns: [
        {
...
describe("stabilizeThreadMessages", () => {
  it("reuses unchanged message objects across rebuilds", () => {
    const firstPass = buildTaskChatMessages({
    ...
    const secondPass = buildTaskChatMessages({
      comments: [
        createComment(),
        createComment({
          id: "comment-2",
