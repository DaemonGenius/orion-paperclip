...
import { buildSubTaskDefaults, buildSubTaskDefaultsForViewer } from "./subTaskDefaults";
...
describe("buildSubTaskDefaults", () => {
  it("inherits the parent agent assignee and workspace context", () => {
    const defaults = buildSubTaskDefaults(
      makeTask({
  ...
  it("leaves the sub-task unassigned when the parent assignee is the current user", () => {
    const defaults = buildSubTaskDefaultsForViewer(
      makeTask({
        assigneeUserId: "user-1",
      }),
      "user-1",
