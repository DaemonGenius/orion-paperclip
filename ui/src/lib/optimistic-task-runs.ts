...
export function upsertInterruptedRun(
  runs: RunForTask[] | undefined,
  run: InterruptRunSource,
  finishedAt: string,
): RunForTask[] {
  const nextRun: RunForTask = {
    runId: run.id,
    status: "cancelled",
    agentId: run.agentId,
...
export function clearTaskExecutionRun(
  task: Task | undefined,
  runId: string,
) {
  if (!task || task.executionRunId !== runId) return task;
  return {
    ...task,
    executionRunId: null,
