...
import { clearTaskExecutionRun, removeLiveRunById, upsertInterruptedRun } from "./optimistic-task-runs";

function createLiveRun(overrides: Partial<LiveRunForTask> = {}): LiveRunForTask {
  return {
    id: "run-1",
    status: "running",
...
describe("clearTaskExecutionRun", () => {
  it("clears the cached execution run when the interrupted run matches the task lock", () => {
    const task = {
      id: "task-1",
    ...
    expect(clearTaskExecutionRun(task, "run-1")).toMatchObject({
      id: "task-1",
      executionRunId: null,
  ...
  it("leaves the cached task alone when another run is interrupted", () => {
    const task = {
      id: "task-1",
    ...
    expect(clearTaskExecutionRun(task, "run-1")).toBe(task);
  });
});
