...
import type { Agent, TaskComment } from "@paperclipai/shared";
import type { ActiveRunForTask, LiveRunForTask } from "../api/heartbeats";
import { formatAssigneeUserLabel } from "./assignees";
import {
  buildTaskThreadInteractionSummary,
  type TaskThreadInteraction,
} from "./task-thread-interactions";
...
export interface TaskChatComment extends TaskComment {
  runId?: string | null;
...
export interface TaskChatLinkedRun {
  runId: string;
  status: string;
  agentId: string;
  adapterType?: string;
...
export interface TaskChatTranscriptEntry {
  kind:
    | "assistant"
    | "thinking"
    | "user"
...
const TASK_CHAT_TRANSCRIPT_MAX_VISIBLE_ENTRIES = 30;
...
function isTaskChatRenderableTranscriptEntry(entry: TaskChatTranscriptEntry) {
...
function compactTaskChatTranscript(
  entries: readonly TaskChatTranscriptEntry[],
  maxVisibleEntries = TASK_CHAT_TRANSCRIPT_MAX_VISIBLE_ENTRIES,
): readonly TaskChatTranscriptEntry[] {
  const renderable = entries
    .map((entry, fullIndex) => ({ entry, fullIndex }))
    .filter(({ entry }) => isTaskChatRenderableTranscriptEntry(entry));
...
function createInteractionMessage(interaction: TaskThreadInteraction) {
  const message: ThreadSystemMessage = {
    id: `interaction:${interaction.id}`,
    role: "system",
    createdAt: toDate(interaction.createdAt),
    content: [{ type: "text", text: buildTaskThreadInteractionSummary(interaction) }],
    metadata: {
      custom: {
        kind: "interaction",
...
function runTimestamp(run: TaskChatLinkedRun) {
...
function computeSegmentTimings(entries: readonly TaskChatTranscriptEntry[]): SegmentTiming[] {
...
function createHistoricalRunMessage(run: TaskChatLinkedRun, agentMap?: Map<string, Agent>) {
...
function createHistoricalTranscriptMessage(args: {
  run: TaskChatLinkedRun;
  transcript: readonly TaskChatTranscriptEntry[];
  hasOutput: boolean;
  agentMap?: Map<string, Agent>;
}) {
  const { run, transcript, hasOutput, agentMap } = args;
  const agentName = run.agentName ?? agentMap?.get(run.agentId)?.name ?? run.agentId.slice(0, 8);
  const compactedTranscript = compactTaskChatTranscript(transcript);
...
export function buildAssistantPartsFromTranscript(entries: readonly TaskChatTranscriptEntry[]): {
...
function normalizeLiveRuns(
  liveRuns: readonly LiveRunForTask[],
...
export function buildTaskChatMessages(args: {
  ...
  interactions?: readonly TaskThreadInteraction[];
  timelineEvents: readonly TaskTimelineEvent[];
  linkedRuns: readonly TaskChatLinkedRun[];
  liveRuns: readonly LiveRunForTask[];
  ...
  for (const run of normalizeLiveRuns(liveRuns, activeRun, taskId)) {
    orderedMessages.push({
      createdAtMs: toTimestamp(run.startedAt ?? run.createdAt),
      order: 3,
