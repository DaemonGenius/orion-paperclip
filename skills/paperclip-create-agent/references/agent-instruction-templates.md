...
|---|---|---|---|
| [`Coder`](agents/coder.md) | Software engineers who implement code, debug tasks, write tests, and coordinate with QA/CTO | `codex_local`, `claude_local`, `cursor`, or another coding adapter | Low (operational) |
...
3. Replace placeholders like `{{companyName}}`, `{{managerTitle}}`, `{{taskPrefix}}`, and URLs.
...
1. Start from `qa.md`.
2. Rewrite the charter around release coordination: assemble release notes from merged PRs, confirm CI is green, tag the release, file follow-up tickets for known tasks.
