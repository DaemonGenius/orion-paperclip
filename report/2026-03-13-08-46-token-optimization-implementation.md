...
Implemented the token-optimization plan across heartbeat orchestration, task context APIs, adapter prompt construction, skill exposure, and agent configuration UX.
...
- Timer and manual wakes now preserve task sessions by default; fresh sessions are forced only for explicit `forceFreshSession` wakes or new task assignment wakes.
...
- Paperclip task context now has incremental APIs: `GET /api/agents/me/inbox-lite`, `GET /api/tasks/:id/heartbeat-context`, and comment delta queries via `GET /api/tasks/:id/comments?after=...&order=asc`.
...
- That likely explains why PAP-507 still showed full task/comment reload behavior even though the incremental context work was already implemented in this branch.
