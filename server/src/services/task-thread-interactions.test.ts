...
function createFakeDb(args: {
  ...
  const taskTouches: Array<Record<string, unknown>> = [];
...
describe("taskThreadInteractionService", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  });
  ...
  it("create reuses an existing interaction for the same idempotency key", async () => {
    const { taskThreadInteractionService } = await import("./task-thread-interactions.js");
    ...
    const svc = taskThreadInteractionService(db as never);
  ...
  it("answerQuestions normalizes duplicate option ids and persists answered results", async () => {
    ...
    const svc = taskThreadInteractionService(state.db as never);
    ...
    expect(state.taskTouches).toHaveLength(1);
  });
});
