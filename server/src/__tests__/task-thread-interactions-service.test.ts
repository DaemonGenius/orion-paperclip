...
import { taskThreadInteractionService } from "../services/task-thread-interactions.js";

const embeddedPostgresSupport = await getEmbeddedPostgresTestSupport();
const describeEmbeddedPostgres = embeddedPostgresSupport.supported ? describe : describe.skip;

describeEmbeddedPostgres("taskThreadInteractionService", () => {
  let db!: ReturnType<typeof createDb>;
  let tasksSvc!: ReturnType<typeof taskService>;
  let interactionsSvc!: ReturnType<typeof taskThreadInteractionService>;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;

  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("paperclip-task-thread-interactions-");
    db = createDb(tempDb.connectionString);
    tasksSvc = taskService(db);
    interactionsSvc = taskThreadInteractionService(db);
  }, 20_000);

  afterEach(async () => {
    await db.delete(taskThreadInteractions);
  ...
  it("accepts suggested tasks by creating a rooted task tree under the current task", async () => {
    ...
    expect(accepted.interaction.result).toMatchObject({
      version: 1,
      createdTasks: [
        expect.objectContaining({ clientKey: "root", parentTaskId: taskId }),
        expect.objectContaining({ clientKey: "child" }),
      ],
    });
    expect(accepted.createdTasks).toEqual([
    ...
    const children = await tasksSvc.list(companyId, { parentId: taskId });
    expect(children).toHaveLength(1);
    expect(children[0]?.title).toBe("Create the root follow-up");

    const nestedChildren = await tasksSvc.list(companyId, { parentId: children[0]!.id });
    ...
    const listed = await interactionsSvc.listForTask(taskId);
    ...
    const childrenAfterDuplicateAccept = await tasksSvc.list(companyId, { parentId: taskId });
  ...
  it("reuses the existing interaction when the same idempotency key is submitted twice", async () => {
    ...
    const rows = await db.select().from(taskThreadInteractions);
    expect(rows).toHaveLength(1);
    expect(rows[0]?.idempotencyKey).toBe("run-1:questionnaire");
  });

  it("accepts request_confirmation interactions without creating child tasks", async () => {
    const companyId = randomUUID();
    const goalId = randomUUID();
    const taskId = randomUUID();
  ...
  it("returns agent-authored request confirmations to the creating agent when a board user accepts", async () => {
    ...
    expect(accepted.continuationTask).toEqual({
      id: taskId,
    ...
    const updatedTask = (await db.select().from(tasks)).find((task) => task.id === taskId);
  ...
  it("expires request confirmations when the watched task document revision changes", async () => {
    const companyId = randomUUID();
    const goalId = randomUUID();
    const taskId = randomUUID();
