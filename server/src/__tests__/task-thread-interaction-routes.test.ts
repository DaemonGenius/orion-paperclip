...
function registerModuleMocks() {
  vi.doMock("../services/index.js", () => ({
    ...
    clampTaskListLimit: (value: number) => value,
    ...
    taskThreadInteractionService: () => mockInteractionService,
...
describe.sequential("task thread interaction routes", () => {
  beforeEach(() => {
    vi.resetModules();
    ...
    registerModuleMocks();
    vi.clearAllMocks();
    mockTaskService.getById.mockResolvedValue(createTask());
    mockInteractionService.listForTask.mockResolvedValue([]);
    ...
    mockInteractionService.acceptInteraction.mockResolvedValue({
      interaction: {
        ...
        result: {
          version: 1,
          createdTasks: [{ clientKey: "task-1", taskId: "child-1" }],
  ...
  it("lists and creates board-authored interactions", async () => {
    ...
    const listRes = await request(app).get("/api/tasks/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/interactions");
    ...
    const createRes = await request(app)
      .post("/api/tasks/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/interactions")
      .send({
        kind: "suggest_tasks",
        payload: {
          version: 1,
  ...
  it("accepts suggested tasks and wakes created assignees plus the current assignee", async () => {
    ...
    const res = await request(app)
      .post("/api/tasks/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/interactions/interaction-1/accept")
  ...
  it("answers questions and emits a continuation wake", async () => {
    ...
    const res = await request(app)
      .post("/api/tasks/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/interactions/interaction-2/respond")
      .send({
        answers: [{ questionId: "scope", optionIds: ["phase-1"] }],
      });
  ...
  it("accepts request confirmations and wakes the current assignee when configured for accept-only wakeups", async () => {
    ...
    const res = await request(app)
      .post("/api/tasks/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/interactions/interaction-3/accept")
      .send({});
  ...
  it("wakes the returned agent when accepting an agent-authored confirmation from a board review assignee", async () => {
    mockTaskService.getById.mockResolvedValueOnce(createTask({
      status: "in_review",
    ...
    const res = await request(app)
      .post("/api/tasks/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/interactions/interaction-4/accept")
      .send({});
  ...
  it("does not emit a continuation wake when request confirmations are rejected", async () => {
    ...
    const res = await request(app)
      .post("/api/tasks/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/interactions/interaction-3/reject")
  ...
  it("does not emit an accept-only continuation wake for rejected suggested tasks", async () => {
    ...
    const res = await request(app)
      .post("/api/tasks/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/interactions/interaction-5/reject")
      .send({ reason: "Not now" });
