...
describeEmbeddedPostgres("stale task execution lock routes", () => {
  ...
  function createApp(actor: Express.Request["actor"]) {
    ...
    app.use("/api", taskRoutes(db, {} as any));
  ...
  it("restricts admin force-release to board users with company access and writes an audit event", async () => {
    ...
    await request(createApp(agentActor(companyId, agentId, currentRunId)))
      .post(`/api/tasks/${taskId}/admin/force-release`)
      .expect(403);
    ...
    const res = await request(createApp(boardActor(companyId)))
      .post(`/api/tasks/${taskId}/admin/force-release?clearAssignee=true`)
      .send();
    ...
    const audit = await db
      ...
      .from(activityLog)
      .where(eq(activityLog.action, "task.admin_force_release"))
      .then((rows) => rows[0]);
