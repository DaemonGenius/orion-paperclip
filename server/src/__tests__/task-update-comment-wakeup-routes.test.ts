...
describe("task update comment wakeups", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("includes the new comment in assignment wakes from task updates", async () => {
    const existing = makeTask();
    const updated = makeTask({
      assigneeAgentId: ASSIGNEE_AGENT_ID,
      assigneeUserId: null,
    });
    mockTaskService.getById.mockResolvedValue(existing);
  ...
  it("wakes the assignee on comment-only task updates", async () => {
    const existing = makeTask({
      assigneeAgentId: ASSIGNEE_AGENT_ID,
      assigneeUserId: null,
      status: "in_progress",
