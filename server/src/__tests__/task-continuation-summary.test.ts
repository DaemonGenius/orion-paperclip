...
import {
  TASK_CONTINUATION_SUMMARY_MAX_BODY_CHARS,
  buildContinuationSummaryMarkdown,
} from "../services/task-continuation-summary.js";

describe("task continuation summaries", () => {
  it("builds bounded task-local handoff context with required sections", () => {
    ...
    expect(body).toContain("- Summary is task-local");
    ...
    expect(body.length).toBeLessThanOrEqual(TASK_CONTINUATION_SUMMARY_MAX_BODY_CHARS);
  });
