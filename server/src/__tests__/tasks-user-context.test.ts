...
import { deriveTaskUserContext } from "../services/tasks.ts";
...
describe("deriveTaskUserContext", () => {
  it("marks task unread when external comments are newer than my latest comment", () => {
    const context = deriveTaskUserContext(
      makeTask({ createdByUserId: "user-1" }),
      "user-1",
      {
  ...
  it("marks task read when my latest comment is newest", () => {
    const context = deriveTaskUserContext(
      makeTask({ createdByUserId: "user-1" }),
      "user-1",
      {
  ...
  it("uses task creation time as fallback touch point for creator", () => {
    const context = deriveTaskUserContext(
      makeTask({ createdByUserId: "user-1", createdAt: new Date("2026-03-06T09:00:00.000Z") }),
      "user-1",
      {
  ...
  it("uses task updated time as fallback touch point for assignee", () => {
    const context = deriveTaskUserContext(
      makeTask({ assigneeUserId: "user-1", updatedAt: new Date("2026-03-06T15:00:00.000Z") }),
      "user-1",
      {
        myLastCommentAt: null,
        myLastReadAt: null,
